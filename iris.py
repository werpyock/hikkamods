...
class IrisMod(loader.Module):
    ...
    async def iris(self, message):
        """Отправляет команду @iris_cm_bot."""
        command_text = utils.get_args_raw(message)
        if not command_text:
            await message.edit("<b>❌ Укажите команду для @iris_cm_bot</b>")
            return

        bot_username = "iris_dark_bot"
        try:
            ...
            await message.edit("<b>❌ Бот не ответил в течение 15 секунд, задача отменена</b>")
